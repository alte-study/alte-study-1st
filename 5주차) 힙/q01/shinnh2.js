...
//의사코드
/*
1. 최소힙 클래스 MinHeap을 만든다.
	1.1. insert: 배열에 자연수를 삽입해야 한다. 
	(bubbleUp은 마지막에 삽입된 배열을 알맞은 자리에 정렬하기 위해 필요)
	1.2. findNDeleteMin: 배열에서 가장 작은 값을 출력하고, 그 값을 배열에서 제거해야 한다. 
...
3. MinHeap의 인스턴스를 생성한다.
4. 입력값의 모음인 input을 순회하며 출력할 값을 추가한다.
	4.1. 숫자가 0인 경우
	> 배열이 비어있다면 0을 출력한다. (empty) 
	> 배열이 비어있지 않다면 가장 작은 값을 출력하고 제거해야 한다. (findNDeleteMin)
...
for (let el of input) {
	if (el == 0) {
		if (minHeap.empty()) output += "0\n";
		else output += `${minHeap.findNDeleteMin()}\n`;
	} else {
		minHeap.insert(el);
	}
}
